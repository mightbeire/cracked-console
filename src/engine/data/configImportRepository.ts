...
function bit(value: boolean | undefined): number {
  return value ? 1 : 0;
}
...
export async function importCommunityConfig(
  ...
  if (await hasImportedPlan()) {
    throw new Error(
      "A plan is already configured. Stage 3 does not replace an active plan.",
    );
  }
  ...
  operation(
    ...
    `INSERT INTO curriculum_versions
     (id, version, title, source_file_name, source_sha256, imported_at,
      day_count, week_count, start_date, end_date)
  ...
  for (let weekIndex = 0; weekIndex < config.plan.weeks.length; weekIndex += 1) {
    ...
    operation(
      ...
      `INSERT INTO curriculum_weeks
       (id, curriculum_version_id, week_number, first_day, last_day,
        date_range_label, title, phase_number, is_consolidation)
    ...
    for (const day of week.days) {
      ...
      operation(
        ...
        `INSERT INTO curriculum_days
         (id, curriculum_version_id, week_id, day_number, date, date_label,
          phase_number, is_rest_day, source_text)
        ...
        [
          ...
          dayId,
          day.date,
          day.label?.trim() || dateLabel(day.date),
          phase,
          bit(day.rest),
        ],
      );
      ...
      for (let blockIndex = 0; blockIndex < day.blocks.length; blockIndex += 1) {
        ...
        operation(
          ...
          `INSERT INTO curriculum_blocks
           (id, day_id, block_type, sort_order, label, planned_minutes,
            instructions_markdown, is_required)
      ...
      for (
        let itemIndex = 0;
        itemIndex < day.definitionOfDone.length;
        itemIndex += 1
      ) {
        operation(
          operations,
          `INSERT INTO definition_of_done_items
           (id, day_id, sort_order, text, is_required)
           VALUES ($1, $2, $3, $4, 1)`,
  ...
  for (let index = 0; index < (config.plan.resources ?? []).length; index += 1) {
    ...
    operation(
      ...
      `INSERT INTO curriculum_resources
       (id, curriculum_version_id, label, url)
       VALUES ($1, 1, $2, $3)`,
      [index + 1, resource.label, resource.url],
    );
  }
  ...
  for (let index = 0; index < config.skills.length; index += 1) {
    ...
    operation(
      operations,
      `INSERT INTO skills (id, code, name, family, sort_order)
       VALUES ($1, $2, $3, $4, $5)`,
      [index + 1, skill.code, skill.name, skill.family, index + 1],
    );
  }
  ...
  for (let index = 0; index < config.projects.length; index += 1) {
    ...
    operation(
      ...
      [
        ...
        project.code,
        project.name,
        project.coreIntegration,
        project.startDay,
        project.endDay,
        project.defenceAssessmentType,
      ],
    );
    ...
    for (
      ...
      operation(
        ...
        `INSERT INTO project_milestones
         (id, project_id, sort_order, title, description)
         VALUES ($1, $2, $3, $4, $5)`,
        [
          milestoneId,
          projectId,
  ...
  if (config.reading) {
    for (let index = 0; index < config.reading.months.length; index += 1) {
      ...
      operation(
        operations,
        `INSERT INTO reading_months
         (id, title, start_date, end_date, required_count)
         VALUES ($1, $2, $3, $4, $5)`,
        [
          index + 1,
          month.title,
    ...
    for (let index = 0; index < config.reading.books.length; index += 1) {
      ...
      operation(
        ...
        `INSERT INTO reading_books
         (id, month_id, slot, title, author, is_short, is_bonus, is_reread,
          bespoke_assignment)
  ...
  if (config.practice) {
    for (
      let stageIndex = 0;
      stageIndex < config.practice.stages.length;
      stageIndex += 1
    ) {
      const stage = config.practice.stages[stageIndex]!;
      operation(
        operations,
        `INSERT INTO practice_stages
         (id, sort_order, title, goal, unlock_after_stage_id)
         VALUES ($1, $2, $3, $4, $5)`,
        [
          stageIndex + 1,
          stageIndex + 1,
    ...
    for (
      ...
      for (
        let lessonIndex = 0;
        lessonIndex < stage.lessons.length;
        lessonIndex += 1
      ) {
        ...
        operation(
          operations,
          `INSERT INTO practice_lessons
           (id, stage_id, sort_order, title, concept, practice_assignment,
            review_prompts)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [
            currentLessonId,
        ...
        for (const resource of lesson.resources ?? []) {
          operation(
            ...
            `INSERT INTO practice_resources
             (id, lesson_id, label, url, resource_type, note)
             VALUES ($1, $2, $3, $4, 'LINK', '')`,
            [resourceId, currentLessonId, resource.label, resource.url],
          );
          resourceId += 1;
        }
    ...
    for (
      let index = 0;
      index < (config.practice.categories ?? []).length;
      index += 1
    ) {
      operation(
        operations,
        `INSERT INTO practice_categories (id, name) VALUES ($1, $2)`,
        [index + 1, config.practice.categories![index]!],
      );
    }

    for (
      let index = 0;
      index < (config.practice.challenges ?? []).length;
      index += 1
    ) {
      ...
      operation(
        ...
        `INSERT INTO practice_challenges
         (id, minimum_stage_order, title, prompt)
         VALUES ($1, $2, $3, $4)`,
        [index + 1, challenge.minimumStage, challenge.title, challenge.prompt],
      );
    }
  }
...
export async function getImportedConfigSummary(): Promise<ConfigSummary | null> {
  ...
  const versionRows = await db.select<Array<{
    title: string;
    version: string;
    startDate: string;
    endDate: string;
  ...
  return {
    ...
    activeDays: await count(
      "SELECT COUNT(*) AS count FROM curriculum_days WHERE is_rest_day = 0",
    ),
    restDays: await count(
      "SELECT COUNT(*) AS count FROM curriculum_days WHERE is_rest_day = 1",
    ),
